...
class BasicConfig(object):
    def __init__(self, grad_accumulate_steps, max_grad_norm, inform_every_batch_num, save_every_epoch_num,
                 use_cuda, learning_rate, epoch_num, metrics, log_dir, ckpt_dir):
...
def run_basics(exp_name, model, dataset, fetch_batch_fn, config):
    ...
    for epoch in range(config.epoch_num):
        loss, train_evaluator = train_fn(model, optimizer, dataset.train_iterator, fetch_batch_fn, device, config)
        print("Finish training {} epochs, loss: {}".format(epoch, loss))
        eval_result = train_evaluator.evaluate(config.metrics)
        writer.add_scalars("train", eval_result, epoch)

        if train_evaluator.is_better_than(best_evaluator, config.metrics):
            print("Best evaluator is updated.")
            best_evaluator = train_evaluator
        ...
        elif epoch & config.save_every_epoch_num == 0:
